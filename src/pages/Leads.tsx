...
import { brl, formatDate, leads } from "@/lib/mock-data";
    ...
    >
      <div className="mb-6 bg-gradient-to-r from-primary to-accent text-primary-foreground rounded-xl p-5 flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-primary-foreground/80">Última sincronização há 12 minutos · 8 novos editais captados hoje</p>
        </div>
            ...
            <tbody>
              {leads.map((l) => (
                <tr key={l.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3"><StatusBadge tone={statusTone(l.status) as never}>{l.status}</StatusBadge></td>
                </tr>
              ))}
            </tbody>
          </table>
