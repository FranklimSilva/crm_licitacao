...
export default function Dashboard() {
  ...
  const { leads, clientes, oportunidades, faturamento } = data ?? { leads: [], clientes: [], oportunidades: [], faturamento: [] };

  const { kpis, maxFat } = useMemo(() => {
    ...
    const totalMes = faturamento[faturamento.length - 1]?.valor ?? 0;
    const max = Math.max(1, ...faturamento.map((f) => f.valor));
    ...
    return { kpis: k, maxFat: max };
  }, [clientes, faturamento, leads, oportunidades]);
  ...
  return (
    <AppShell title="Dashboard" subtitle="Resumo executivo das operações de licitação">
      ...
      {!data ? (
        <div className="bg-card border border-border rounded-xl p-6 text-sm text-muted-foreground">Carregando dados...</div>
      ) : (
        <>
              ...
              <div className="space-y-3">
                {leads.slice(0, 5).map((l) => (
                  ...
                  >
                    ...
                    <StatusBadge
                      tone={l.status === "Qualificado" ? "success" : l.status === "Descartado" ? "muted" : "info"}
                    >
                      {l.status}
                    </StatusBadge>
                  </div>
